...
curl -s https://crt.sh/\?q\=\%.$1\&output\=json | jq -r '.[].name_value' | sed 's/\*\.//g' | sort -u | httprobe | tee -a ./sec.txt
}
...
python3 ~/tools/dirsearch/dirsearch.py -u $1 -e $2 -t 100 -H 'X-FORWARDED-FOR: 127.0.0.1'
}
